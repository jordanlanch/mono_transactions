defmodule TransactionsMonoWeb.Api.TransactionsController do
  ...
  def create(
    ...
    {:ok, %Transactions{id: id}} <-
      HelperTransactions.create_transactions(transaction_params_ok) do
        ...
        conn
        |> put_status(:created)
        |> put_resp_header("location", Routes.api_transactions_path(conn, :show, id, transaction.id))
        |> render("show.json", transaction: transaction)
      end
  end

  def show(%{private: %{:guardian_default_resource => %TransactionsMono.Accounts.User{id: id}}} =
    conn, %{"id_transaction" => id_transaction, "id_user" => id_user}) do
  ...
  defp add_ids_to_transaction(%{"email" => email}= transactions_params, id_user) do
    case Accounts.get_user_by_email(email) do
      %User{} = user -> {:ok, transactions_params |> Map.put("user_from_id", id_user) |> Map.put("user_to_id", user.id)}
      nil -> {:error, "Email don't exists"}
    end
